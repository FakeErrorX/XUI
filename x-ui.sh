...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/FakeErrorX/XUI/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_menu() {
    ...
    wget -O /usr/bin/x-ui https://raw.githubusercontent.com/FakeErrorX/XUI/main/x-ui.sh
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/fakeerrorx/xui/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/fakeerrorx/xui/master/install.sh)${plain}"
    echo ""
...
update_shell() {
    wget -O /usr/bin/x-ui -N https://github.com/FakeErrorX/XUI/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
show_menu() {
    echo -e "
╔────────────────────────────────────────────────╗
│   ${green}XUI Panel Management Script${plain}                │
